...
export default function ProfilePage() {
  ...
  const cardStyle: React.CSSProperties = {
    background: 'white', borderRadius: '20px', border: '1.5px solid #f0f0f0',
    padding: '1.75rem', boxShadow: '0 4px 16px rgba(0,0,0,0.05)',
  }
  ...
  return (
    <div style={{ minHeight: 'calc(100vh - 64px)', background: '#f8f9fc', padding: isMobile ? '1.5rem 1rem' : '2rem 1.5rem' }}>
      <div style={{ maxWidth: '520px', margin: '0 auto', display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
        ...
        <div>
          <h1 style={{ fontSize: isMobile ? '1.125rem' : '1.375rem', fontWeight: 700, color: '#111827', margin: 0 }}>마이페이지</h1>
          <p style={{ color: '#9ca3af', fontSize: '0.875rem', marginTop: '0.25rem' }}>계정 정보를 확인하고 수정할 수 있어요</p>
        </div>

        {/* 계정 정보 */}
        <div style={cardStyle}>
          <h2 style={{ fontSize: '0.9375rem', fontWeight: 700, color: '#111827', margin: '0 0 1.25rem 0' }}>계정 정보</h2>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', padding: '0.75rem 1rem', background: '#f8f9fc', borderRadius: '12px' }}>
              <span style={{ fontSize: '0.875rem', color: '#6b7280' }}>이메일</span>
              <span style={{ fontSize: '0.875rem', color: '#111827', fontWeight: 500 }}>{user.email}</span>
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between', padding: '0.75rem 1rem', background: '#f8f9fc', borderRadius: '12px' }}>
              <span style={{ fontSize: '0.875rem', color: '#6b7280' }}>닉네임</span>
              <span style={{ fontSize: '0.875rem', color: '#111827', fontWeight: 500 }}>{user.nickname}</span>
            </div>
          </div>
        </div>
        ...
        {/* 닉네임 변경 */}
        <div style={cardStyle}>
          <h2 style={{ fontSize: '0.9375rem', fontWeight: 700, color: '#111827', margin: '0 0 1.25rem 0' }}>닉네임 변경</h2>
          <form onSubmit={handleNicknameSave} style={{ display: 'flex', flexDirection: 'column', gap: '0.875rem' }}>
            ...
            <button
              type="submit" disabled={isNicknameSaving}
        ...
        {/* 비밀번호 변경 */}
        <div style={cardStyle}>
          <h2 style={{ fontSize: '0.9375rem', fontWeight: 700, color: '#111827', margin: '0 0 1.25rem 0' }}>비밀번호 변경</h2>
          <form onSubmit={handlePasswordSave} style={{ display: 'flex', flexDirection: 'column', gap: '0.875rem' }}>
            ...
            />
            <input
              type="password" value={pw.next} onChange={(e) => setPw({ ...pw, next: e.target.value })}
            ...
            <button
              type="submit" disabled={isPwSaving}
        ...
        <div style={cardStyle}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <div>
              <p style={{ fontWeight: 600, color: '#111827', fontSize: '0.9375rem', margin: 0 }}>로그아웃</p>
              <p style={{ color: '#9ca3af', fontSize: '0.8125rem', margin: '0.25rem 0 0 0' }}>다른 계정으로 로그인하려면 로그아웃하세요</p>
            </div>
            <button
              onClick={logout}
              style={{ fontSize: '0.875rem', color: '#6b7280', padding: '0.5rem 1rem', border: '1.5px solid #e5e7eb', borderRadius: '10px', background: 'white', cursor: 'pointer', whiteSpace: 'nowrap' }}
            >
              로그아웃
            </button>
          </div>
        ...
        <div style={{ ...cardStyle, border: '1.5px solid #fee2e2' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <div>
              ...
              <p style={{ color: '#9ca3af', fontSize: '0.8125rem', margin: '0.25rem 0 0 0' }}>탈퇴하면 모든 페르소나와 대화 기록이 삭제돼요</p>
            </div>
            <button
              onClick={async () => {
              ...
              style={{ fontSize: '0.875rem', color: '#dc2626', padding: '0.5rem 1rem', border: '1.5px solid #fecaca', borderRadius: '10px', background: 'white', cursor: 'pointer', whiteSpace: 'nowrap' }}
            >
              탈퇴하기
            </button>
          </div>
