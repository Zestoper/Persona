...
export default function CreatePersonaPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await api.post('/personas', { ...form, avatar_url: avatarUrl })
    ...
    <div style={{ minHeight: 'calc(100vh - 56px)', background: '#f8f9fc', padding: '2rem 1.5rem' }}>
      <div style={{ maxWidth: '580px', margin: '0 auto' }}>
        ...
        <div style={{ marginBottom: '1.5rem' }}>
          <h1 style={{ fontSize: '1.375rem', fontWeight: 700, color: '#111827', margin: 0 }}>페르소나 만들기</h1>
          <p style={{ color: '#9ca3af', fontSize: '0.875rem', marginTop: '0.375rem' }}>캐릭터 정보를 입력하면 AI 설정이 자동으로 생성돼요</p>
        </div>
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.375rem' }}>
                  {STYLES.map((s) => (
                    <button
                      ...
                      style={{
                        padding: '0.3rem 0.75rem', borderRadius: '999px', fontSize: '0.75rem', fontWeight: 600, cursor: 'pointer', border: 'none',
                        background: style === s.id && !avatarFile ? 'linear-gradient(135deg, #6366f1, #8b5cf6)' : '#f3f4f6',
                        color: style === s.id && !avatarFile ? 'white' : '#6b7280',
                      }}
                    >
                      {s.label}
                    </button>
